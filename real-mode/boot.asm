...
step2:
    ...
    mov ah, 2       ;read sectors command(interrupt 13)
    mov al, 1       ;number of sectors to read
    mov ch, 0       ;cylinder number
    mov cl, 2       ;sector number
    mov dh, 0       ;head number. the dl already contains the drive number approphiately
...
error:
    mov si,error_message        ;printing error msg if error occurs
    call print
    jmp $
...
buffer:             ;this is a ref to the data message we stored just after our boot sector. Here we cant do db and store a message becasue this is
                    ;after our boot sector and BIOS only loads 1 sector(512 B). But, we can always ref it like this and use it.